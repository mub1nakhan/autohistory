"""
Vehicles app models.
Core domain entity representing a vehicle in the AutoProof system.
"""
...
class Vehicle(models.Model):
    """
    Core vehicle entity. Identified uniquely by VIN code.
    """
    ...
    ownership_count = models.PositiveSmallIntegerField(
        default=1, help_text="Number of previous owners"
    )
    ...
    search_vector = models.GeneratedField(  # Django 5+
        expression=None,  # Set via signal/migration
        output_field=models.TextField(),
        db_persist=False,
    ) if False else None  # Managed via PostgreSQL trigram
...
class VehicleOwnershipHistory(models.Model):
    """Tracks every ownership change for a vehicle."""

    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    vehicle = models.ForeignKey(Vehicle, on_delete=models.CASCADE, related_name="ownership_history")
    owner = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True)
    owner_name = models.CharField(max_length=200, help_text="Snapshot of owner name at time of transfer")
